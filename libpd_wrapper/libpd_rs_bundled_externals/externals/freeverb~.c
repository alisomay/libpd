...
static const int combtuningR[numcombs] = {1116 + stereospread, 1188 + stereospread, 1277 + stereospread, 1356 + stereospread,
...
static inline t_float comb_processL(t_freeverb *x, int filteridx, t_float input)
{
    ...
    if (++x->x_combidxL[filteridx] >= x->x_combtuningL[filteridx])
        x->x_combidxL[filteridx] = 0;
...
static inline t_float comb_processR(t_freeverb *x, int filteridx, t_float input)
{
    ...
    if (++x->x_combidxR[filteridx] >= x->x_combtuningR[filteridx])
        x->x_combidxR[filteridx] = 0;
